import type { FileChooser, Frame, Locator, Page } from "./browser-driver";
...
export async function fillFormField(page: Page, field: BrowserFillField) {
  ...
  if (isSplLocationAutocompleteField(normalizedField)) {
    const splLocationFilled = await fillSplLocationAutocomplete(page, normalizedField);

    if (splLocationFilled !== undefined) {
      return splLocationFilled;
    }
  }
  ...
  if (strategy === "autocomplete") {
    if (shouldUseSelectLikeFlow(normalizedField) && await fillSelectLikeField(page, normalizedField)) {
      return true;
    }
...
export async function collectLegacyFillFieldDebug(page: Page, field: BrowserFillField) {
  ...
  for (const frame of page.frames()) {
    ...
    for (const target of targets) {
      ...
      const interactionTarget = await getSelectInteractionTarget(target).catch(() => target);
      const control = await interactionTarget.evaluate((element) => {
      ...
      const popupScope = await waitForLegacyDropdownScope(frame, interactionTarget, 250);
      const optionTexts = popupScope ? await readLegacyVisibleOptionTexts(popupScope) : [];
      const highlightedText = popupScope ? await readLegacyHighlightedOption(popupScope) : "";
  const popupState = popupScope
    ? await popupScope.frame.evaluate((popupMarker) => {
      ...
      function findPopupByMarker(marker: string) {
        for (const root of getSearchRoots()) {
          const match = root.querySelector(`[data-gradlaunch-legacy-dropdown-popup="${CSS.escape(marker)}"]`);
    ...
    : undefined;
      ...
      if (popupScope) {
        await cleanupLegacyDropdownScope(popupScope);
      }
...
async function fillClassifiedSelectLike(page: Page, frame: Frame, target: Locator, field: BrowserFillField) {
  ...
  const isLocationSelectLike = isLocationLikeDescriptor(normalizeInline(`${field.label} ${field.value}`));
  ...
  if (!isLocationSelectLike && await clickVisibleSelectOptionWithRetries(frame, field.value, field.label, field.value, 4)) {
    await interactionTarget.press("Tab", { timeout: 350 }).catch(() => undefined);
    await interactionTarget.blur().catch(() => undefined);
    await page.waitForTimeout(260).catch(() => undefined);
  ...
  for (const query of getSelectLikeQueries(field.label, field.value)) {
    ...
    const clicked = isLocationSelectLike
      ? await clickLocationSuggestionByText(frame, field.value, field.label, query)
      : wantedCountry && await clickExactCountryOption(frame, wantedCountry)
...
async function fillClassifiedAutocomplete(frame: Frame, target: Locator, field: BrowserFillField) {
  ...
  const isLocationAutocomplete = isLocationLikeDescriptor(normalizeInline(`${field.label} ${field.value}`));
  ...
  try {
    for (const query of getSelectLikeQueries(field.label, field.value)) {
      ...
      await target.type(query, { delay: 35, timeout: 1800 }).catch(() => undefined);
      await frame.page().waitForTimeout(350).catch(() => undefined);

      const clicked = isLocationAutocomplete
        ? await clickLocationSuggestionByText(frame, field.value, field.label, query)
        : await clickBestAutocompleteSuggestion(frame, marker, field.value, field.label, query);
...
async function fillClassifiedTextLike(target: Locator, field: BrowserFillField, kind: CanonicalFieldKind) {
  ...
  if (kind === "phone" || isPhoneLikeFillField(field)) {
    return fillIntlPhoneLocator(target, { ...field, value });
  }

  return commitTextLikeLocator(target, { ...field, value }, kind);
}

async function fillIntlPhoneLocator(target: Locator, field: BrowserFillField) {
  const phone = parsePhoneValueForWidget(field.value);

  if (!phone) {
    return false;
  }

  if (await typeInternationalPhoneNumber(target, field, phone)) {
    return true;
  }

  const marker = `gl-phone-country-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const setup = await target.evaluate((element, { marker, phone }) => {
    const targetElement = findPhoneInput(element instanceof HTMLElement ? element : undefined);

    if (!targetElement) {
      return { attempted: false, needsClick: false };
    }

    const container = findPhoneContainer(targetElement);
    const hasPhoneWidget = Boolean(
      container?.querySelector(".iti__selected-dial-code, .iti__selected-flag, [class*='iti__'], mat-select, [role='combobox'], [aria-haspopup]")
      || /\b(iti|intl|phone|tel|country|dial|mat-mdc-form-field|mat-form-field)\b/i.test(String(container?.className ?? ""))
    );
    ...
    let countryCommitted = false;
    const win = window as typeof window & {
      intlTelInputGlobals?: {
        getInstance?: (input: HTMLInputElement) => {
    ...
    const instance = win.intlTelInputGlobals?.getInstance?.(targetElement);

    if (instance && phone.countryIso) {
      instance.setCountry?.(phone.countryIso);
      instance.setNumber?.(phone.fullNumber);
      countryCommitted = instance.getSelectedCountryData?.().dialCode === phone.dialCode.replace(/^\+/, "");
    } else if (instance) {
      instance.setNumber?.(phone.fullNumber);
      countryCommitted = instance.getSelectedCountryData?.().dialCode === phone.dialCode.replace(/^\+/, "");
	    }

    countryCommitted = setNativeCountrySelect(container, phone) || countryCommitted;
    setRelatedHiddenPhoneValues(container, phone);

    const selectedDial = collectSelectedDial(container);
    const dialMatches = selectedDial ? normalizeDial(selectedDial) === phone.dialCode : countryCommitted;

    if (countryCommitted || dialMatches) {
      if (!targetElement.value.trim()) {
        setNativeValue(targetElement, phone.localNumber);
      ...
      return {
        attempted: true,
        needsClick: false,
        filled: Boolean(targetElement.value.trim())
      };
    }

    if (selectedDial && normalizeDial(selectedDial) !== phone.dialCode) {
      const trigger = findCountryTrigger(container);

      if (trigger) {
        setNativeValue(targetElement, phone.localNumber);
        dispatch(targetElement);
        trigger.setAttribute("data-gradlaunch-phone-country-trigger", marker);
        return { attempted: true, needsClick: true, localNumber: phone.localNumber };
      }
    }
    ...
    return {
      attempted: true,
      needsClick: false,
      filled: Boolean(targetElement.value.trim()) && (countryCommitted || dialMatches)
    };

    function findPhoneInput(element: HTMLElement | undefined) {
      if (element instanceof HTMLInputElement) {
        return element;
      }

      return element?.querySelector("input[type='tel'], input[inputmode='tel'], input") instanceof HTMLInputElement
        ? element.querySelector("input[type='tel'], input[inputmode='tel'], input") as HTMLInputElement
        : undefined;
    }

    function findPhoneContainer(input: HTMLInputElement) {
      let current: HTMLElement | null = input;
      let fallback: HTMLElement = input;

      for (let depth = 0; depth < 7 && current; depth += 1) {
        const className = String(current.getAttribute("class") ?? "");
        const hasPhoneSelector = current.querySelector(".iti__selected-dial-code, .iti__selected-flag, [class*='iti__'], mat-select, [role='combobox'], [aria-haspopup]");

        fallback = current;

        if (hasPhoneSelector || /\b(iti|intl|phone|tel|country|dial|mat-mdc-form-field|mat-form-field)\b/i.test(className)) {
          return current;
        }
      ...
      return fallback;
	    ...
	    function setNativeCountrySelect(container: HTMLElement, phoneValue: typeof phone) {
      const selects = Array.from(container.querySelectorAll("select")) as HTMLSelectElement[];

      for (const select of selects) {
	        const option = Array.from(select.options).find((item) => {
	          const optionText = normalize(`${item.textContent ?? ""} ${item.value}`);
	          return (phoneValue.countryName ? optionText.includes(normalize(phoneValue.countryName)) : false)
	            || optionText.includes(phoneValue.dialCode.replace(/^\+/, ""))
	            || (phoneValue.countryIso ? optionText === phoneValue.countryIso : false);
	        });
    ...
    function setRelatedHiddenPhoneValues(container: HTMLElement, phoneValue: typeof phone) {
      const hiddenInputs = Array.from(container.querySelectorAll("input[type='hidden']")) as HTMLInputElement[];

      for (const input of hiddenInputs) {
        const descriptor = normalize(`${input.name} ${input.id} ${input.getAttribute("formcontrolname") ?? ""}`);

	        if (phoneValue.countryIso && /\b(country|iso)\b/.test(descriptor)) {
	          setNativeValue(input, phoneValue.countryIso);
	          dispatch(input);
	        } else if (/\b(dial|code|prefix)\b/.test(descriptor)) {
          setNativeValue(input, phoneValue.dialCode);
          dispatch(input);
        } else if (/\b(phone|mobile|tel)\b/.test(descriptor)) {
          setNativeValue(input, phoneValue.fullNumber);
          dispatch(input);
        }
      }
    }

    function findCountryTrigger(container: HTMLElement) {
      const triggers = Array.from(container.querySelectorAll(".iti__selected-flag, .iti__selected-country, mat-select, [role='combobox'], [aria-haspopup], button, [tabindex]")) as HTMLElement[];
      ...
      return triggers.find((item) => {
        ...
        return /\+\d{1,4}\b|country|dial|code|flag|select/.test(text)
          || /\b(iti|country|dial|phone|mat-select)\b/i.test(String(item.getAttribute("class") ?? ""));
      });
    }

    function collectSelectedDial(container: HTMLElement) {
      const instanceDial = getIntlTelInputDial(targetElement);
      ...
      for (const candidate of candidates) {
        if (!isVisible(candidate) || isInOptionList(candidate)) {
          continue;
        }

        const dial = extractDial([
    ...
    function getIntlTelInputDial(input: HTMLInputElement | undefined) {
      if (!(input instanceof HTMLInputElement)) {
        return "";
      }

      const instanceData = win.intlTelInputGlobals?.getInstance?.(input)?.getSelectedCountryData?.();
      const dialDigits = instanceData?.dialCode?.replace(/\D+/g, "");

      return dialDigits ? `+${dialDigits}` : "";
    }

    function extractDial(values: Array<string | null | undefined>) {
      for (const value of values) {
        const dial = value?.match(/\+\d{1,4}/)?.[0];
    ...
    function isInOptionList(element: HTMLElement) {
      return Boolean(element.closest("[role='listbox'], [role='menu'], .iti__country-list, .iti__country, .cdk-overlay-pane, mat-option, [class*='option']"));
    }
  ...
  }, { marker, phone }).catch(() => ({ attempted: false, needsClick: false }));
  ...
  if (setup.needsClick) {
    ...
    const trigger = page.locator(`[data-gradlaunch-phone-country-trigger="${marker}"]`).first();

    try {
      await trigger.click({ force: true, timeout: 700 });
      await page.waitForTimeout(180).catch(() => undefined);
      if (!await clickPhoneCountryOption(page, phone)) {
        return false;
      }
    } catch (_error) {
      return false;
      ...
      await trigger.evaluate((element) => {
        if (element instanceof HTMLElement) {
          element.removeAttribute("data-gradlaunch-phone-country-trigger");
        }
      }).catch(() => undefined);
    }
    ...
    await target.press("Tab", { timeout: 350 }).catch(() => undefined);
    await target.blur().catch(() => undefined);
    await page.waitForTimeout(240).catch(() => undefined);
  }

  if (await verifyLocatorCommitted(target, field, { kind: "phone", allowPartial: false })) {
    return true;
  }

  if (await openPhoneCountryDropdown(target) && await clickPhoneCountryOption(target.page(), phone)) {
    await target.press("Tab", { timeout: 350 }).catch(() => undefined);
    await target.blur().catch(() => undefined);
    await target.page().waitForTimeout(180).catch(() => undefined);
    return verifyLocatorCommitted(target, field, { kind: "phone", allowPartial: false });
  }

  return false;
}

async function openPhoneCountryDropdown(target: Locator) {
  const marker = `gl-phone-country-reopen-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const found = await target.evaluate((element, marker) => {
    const targetElement = element instanceof HTMLInputElement
      ? element
      : element instanceof HTMLElement
        ? element.querySelector("input[type='tel'], input[inputmode='tel'], input")
        : undefined;
    ...
    const container = findPhoneContainer(targetElement);
    const trigger = container ? findCountryTrigger(container) : undefined;

    if (!trigger) {
      return false;
    }

    trigger.setAttribute("data-gradlaunch-phone-country-reopen", marker);
    return true;
    ...
    function findPhoneContainer(input: HTMLInputElement) {
      ...
      for (let depth = 0; depth < 7 && current; depth += 1) {
        ...
        if (
          current.querySelector(".iti__selected-dial-code, .iti__selected-flag, [class*='iti__'], mat-select, [role='combobox'], [aria-haspopup]")
          || /\b(iti|intl|phone|tel|country|dial|mat-mdc-form-field|mat-form-field)\b/i.test(className)
        ) {
          return current;
        }
  ...
  const trigger = target.page().locator(`[data-gradlaunch-phone-country-reopen="${marker}"]`).first();
  ...
  try {
    ...
    await trigger.evaluate((element) => {
      if (element instanceof HTMLElement) {
        element.removeAttribute("data-gradlaunch-phone-country-reopen");
      }
    }).catch(() => undefined);
  }
}

async function typeInternationalPhoneNumber(target: Locator, field: BrowserFillField, phone: ParsedPhoneValue) {
  ...
  try {
    ...
    await target.type(phone.fullNumber, { delay: 18, timeout: Math.max(1600, phone.fullNumber.length * 80) });
    await target.press("Tab", { timeout: 350 }).catch(() => undefined);
    await target.blur().catch(() => undefined);
    await target.page().waitForTimeout(180).catch(() => undefined);

    if (await verifyLocatorCommitted(target, field, { kind: "phone", allowPartial: false })) {
      await target.page().waitForTimeout(160).catch(() => undefined);
      return verifyLocatorCommitted(target, field, { kind: "phone", allowPartial: false });
    }
  } catch (_error) {
    return false;
...
async function clickPhoneCountryOption(page: Page, phone: ParsedPhoneValue) {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    if (await searchAndClickPhoneCountryOption(page, phone) || await clickVisiblePhoneCountryOption(page, phone)) {
      await closePhoneCountryDropdown(page);
      return true;
    }
...
async function searchAndClickPhoneCountryOption(page: Page, phone: ParsedPhoneValue) {
  for (const query of phone.searchQueries) {
    if (await typePhoneCountrySearch(page, query)) {
      await page.waitForTimeout(120).catch(() => undefined);

      if (await clickVisiblePhoneCountryOption(page, phone)) {
        return true;
      }
    }
  }
...
async function typePhoneCountrySearch(page: Page, query: string) {
  const marker = `gl-phone-country-search-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

  for (const frame of page.frames()) {
	    const found = await frame.evaluate((marker) => {
	      const candidates = Array.from(document.querySelectorAll([
	        ".iti__search-input",
	        ".iti input[type='search']",
        ".iti input[placeholder*='Search' i]",
        ".iti input[aria-label*='Search' i]",
        "[class*='country'] input[type='search']",
        "[class*='country'] input[placeholder*='Search' i]",
        ...
        "[role='dialog'] input[placeholder*='Search' i]"
	      ...
	      for (const input of candidates) {
	        ...
	        const container = input.closest(".iti, [class*='country'], [role='dialog'], [role='listbox'], [role='menu']") as HTMLElement | null;
	        const text = normalize(container?.innerText || container?.textContent || "");
	        const dialMatches = text.match(/\+\d{1,4}/g) ?? [];
	        const hasCountryOptionMarkup = Boolean(container?.querySelector("[data-dial-code], [data-country-code], .iti__country, [class*='country-list'], [role='option']"));

	        if (!hasCountryOptionMarkup && dialMatches.length < 3) {
	          continue;
	        }

        input.setAttribute("data-gradlaunch-phone-country-search", marker);
        return true;
      }
    ...
    const searchInput = frame.locator(`[data-gradlaunch-phone-country-search="${marker}"]`).first();

    try {
      await searchInput.click({ force: true, timeout: 700 }).catch(() => undefined);
      await searchInput.press(process.platform === "darwin" ? "Meta+A" : "Control+A", { timeout: 250 }).catch(() => undefined);
      await searchInput.press("Backspace", { timeout: 250 }).catch(() => undefined);
      await searchInput.type(query, { delay: 12, timeout: 1000 });
      return true;
    } finally {
      await searchInput.evaluate((input) => {
        if (input instanceof HTMLElement) {
          input.removeAttribute("data-gradlaunch-phone-country-search");
        }
      }).catch(() => undefined);
    }
  }
...
async function closePhoneCountryDropdown(page: Page) {
  await page.keyboard.press("Escape").catch(() => undefined);
  await page.waitForTimeout(80).catch(() => undefined);
}

async function clickVisiblePhoneCountryOption(page: Page, phone: ParsedPhoneValue) {
  for (const frame of page.frames()) {
    const clicked = await frame.evaluate((phone) => {
      const dialDigits = phone.dialCode.replace(/^\+/, "");
      const directSelectors = [
        phone.countryIso ? `[data-country-code="${CSS.escape(phone.countryIso)}"]` : undefined,
        `[data-dial-code="${CSS.escape(dialDigits)}"]`,
        phone.countryIso ? `.iti__country[data-country-code="${CSS.escape(phone.countryIso)}"]` : undefined,
        `.iti__country[data-dial-code="${CSS.escape(dialDigits)}"]`
      ].filter((selector): selector is string => Boolean(selector));
      const directCandidates = Array.from(document.querySelectorAll(directSelectors.join(","))) as HTMLElement[];
      ...
      const candidates = [...directCandidates, ...broadCandidates];
      ...
      for (const candidate of candidates) {
        ...
        const countryCode = normalize(candidate.getAttribute("data-country-code") ?? "");
        const dialCode = (candidate.getAttribute("data-dial-code") ?? "").replace(/\D+/g, "");
        ...
        let score = 0;
	        ...
	        if (phone.countryIso && countryCode === phone.countryIso) {
	          score += 220;
	        }

        if (dialCode === dialDigits) {
          score += 200;
        }

        if (candidate.className.toString().includes("iti__country")) {
          score += 50;
        }

	        if (phone.countryName && text.includes(normalize(phone.countryName))) {
	          score += 80;
	        }

        if (text.includes(phone.dialCode) || text.includes(`+${dialDigits}`)) {
          score += 75;
        }
        ...
        if (candidate.closest("[role='listbox'], [role='menu'], .cdk-overlay-pane, [class*='option'], [class*='dropdown'], [class*='country-list']")) {
          score += 25;
        }
      ...
      const clickTarget = getClickTarget(best);

      clickTarget.scrollIntoView({ block: "center", inline: "nearest" });
      dispatchMouse(clickTarget, "mousedown");
      dispatchMouse(clickTarget, "mouseup");
      clickTarget.click();
      ...
      function getClickTarget(element: HTMLElement) {
        return element.closest("li, [role='option'], [role='menuitem'], mat-option, button, [tabindex]") as HTMLElement | null ?? element;
      }

      function dispatchMouse(element: HTMLElement, type: string) {
        try {
          element.dispatchEvent(new PointerEvent(type === "mousedown" ? "pointerdown" : "pointerup", { bubbles: true, cancelable: true, view: window }));
        } catch (_error) {
...
function parsePhoneValueForWidget(value: string): ParsedPhoneValue | undefined {
  ...
  if (!cleanValue) {
	...
	const explicitSeparatedDial = cleanValue.match(/^\s*\+(\d{1,4})(?=$|[\s().-])/)?.[1];
	const compactExplicitDial = cleanValue.match(/^\s*\+(\d{1,4})/)?.[1];
	const country = explicitSeparatedDial
	  ? findPhoneCountryByExactDial(explicitSeparatedDial) ?? createGenericPhoneCountry(explicitSeparatedDial)
	  : findPhoneCountryByPrefix(digits)
	    ?? inferDefaultPhoneCountry(digits)
	    ?? (compactExplicitDial ? createGenericPhoneCountry(compactExplicitDial) : undefined);
  ...
  if (localNumber.length < 6) {
    return undefined;
	  ...
	  return {
	    fullNumber: `${country.dialCode}${localNumber}`,
	    localNumber,
	    dialCode: country.dialCode,
	    countryIso: country.countryIso,
	    countryName: country.countryName,
	    searchQueries: buildPhoneCountrySearchQueries(country)
	  };
	}

	function findPhoneCountryByExactDial(digits: string) {
	  const normalizedDial = `+${digits.replace(/\D+/g, "")}`;
	  return PHONE_COUNTRIES.find((country) => country.dialCode === normalizedDial);
	}
...
function findPhoneCountryByPrefix(digits: string) {
  ...
  return PHONE_COUNTRIES
	...
	function inferDefaultPhoneCountry(digits: string) {
	  return digits.length === 10 ? INDIA_PHONE_COUNTRY : undefined;
	}

function createGenericPhoneCountry(digits: string): PhoneCountry | undefined {
	  const dialDigits = digits.replace(/\D+/g, "").slice(0, 4);

	  if (!dialDigits) {
	    return undefined;
	  }

	  return { dialCode: `+${dialDigits}` };
	}

function buildPhoneCountrySearchQueries(country: PhoneCountry) {
	  const queries = [
	    country.countryName,
	    country.countryIso,
	    country.dialCode,
...
const INDIA_PHONE_COUNTRY: PhoneCountry = { dialCode: "+91", countryIso: "in", countryName: "India" };

const PHONE_COUNTRIES: PhoneCountry[] = [
  INDIA_PHONE_COUNTRY,
  { dialCode: "+1", countryIso: "us", countryName: "United States" },
  { dialCode: "+44", countryIso: "gb", countryName: "United Kingdom" },
  { dialCode: "+61", countryIso: "au", countryName: "Australia" },
  { dialCode: "+971", countryIso: "ae", countryName: "United Arab Emirates" },
  { dialCode: "+65", countryIso: "sg", countryName: "Singapore" },
  { dialCode: "+49", countryIso: "de", countryName: "Germany" },
  { dialCode: "+33", countryIso: "fr", countryName: "France" },
  { dialCode: "+81", countryIso: "jp", countryName: "Japan" },
  { dialCode: "+82", countryIso: "kr", countryName: "South Korea" }
];
...
async function commitTextLikeLocator(target: Locator, field: BrowserFillField, kind: CanonicalFieldKind | FillStrategy = "text") {
  ...
  const retryCommitted = await target.click({ force: true, timeout: 900 })
    .then(async () => {
      await target.press(selectAllKey, { timeout: 350 }).catch(() => undefined);
      await target.press("Delete", { timeout: 350 }).catch(() => undefined);
      await target.type(value, { delay: 38, timeout: Math.max(2600, value.length * 100) });
      await target.press("Tab", { timeout: 450 }).catch(() => undefined);
      await target.blur().catch(() => undefined);
  ...
  return target.evaluate((element, { expectedValue, fieldLabel, kind, allowPartial, acceptLocationTextValue }) => {
    ...
    if (element instanceof HTMLInputElement && element.type === "date") {
      return Boolean(element.value);
	    ...
	    if ((kind === "phone" || (element instanceof HTMLInputElement && element.type === "tel") || /\b(phone|mobile|telephone|contact)\b/.test(label)) && hasPhoneExpectedValue(expectedValue)) {
	      return phoneValueMatches(element, expectedValue);
	    }
    ...
    function collectCommittedValues(targetElement: HTMLElement, fieldKind: string | undefined) {
      ...
      if (fieldKind === "phone" || (targetElement instanceof HTMLInputElement && targetElement.type === "tel")) {
        const selectedDial = collectPhoneDialText(targetElement, container);

        if (targetElement instanceof HTMLInputElement || targetElement instanceof HTMLTextAreaElement) {
          values.push(`${selectedDial} ${targetElement.value}`);
        }
      }
    ...
    function matchesExpected(actual: string, expectedValue: string, labelText: string, matchOptions: { allowPartial?: boolean; allowLocationAlias?: boolean }) {
      ...
      if ((kind === "phone" || /\b(phone|mobile|telephone|contact)\b/.test(labelText)) && hasPhoneExpectedValue(expectedValue)) {
        return phoneDigitsMatch(actual, expectedValue);
      }
      ...
      return getLocationAliases(expectedValue).some((alias) => alias.length > 2 && (hasPhrase(actual, alias) || (matchOptions.allowPartial && hasPhrase(alias, actual))));
	    ...
	    function phoneValueMatches(targetElement: HTMLElement, expectedValue: string) {
	      const expectedPhone = parseExpectedPhone(expectedValue);

	      if (!expectedPhone) {
	        return false;
	      }

	      const phoneInput = findPhoneTextInput(targetElement);
	      const rawValue = phoneInput?.value
	        ?? (targetElement instanceof HTMLInputElement || targetElement instanceof HTMLTextAreaElement ? targetElement.value : targetElement.textContent ?? "");
	      const rawDigits = rawValue.replace(/\D+/g, "");

	      if (!rawDigits.endsWith(expectedPhone.localDigits)) {
	        return false;
	      }

	      if (!expectedPhone.dialCode) {
	        return true;
	      }

	      const verificationTarget = phoneInput ?? targetElement;
	      const phoneContainer = findPhoneContainer(verificationTarget) ?? findFieldContainer(verificationTarget) ?? verificationTarget.parentElement;
	      const labelDial = fieldLabel.trim().match(/^\+\d{1,4}$/)?.[0] ?? "";
	      const selectedDial = normalizePhoneDial(collectPhoneDialText(verificationTarget, phoneContainer) || labelDial);
	      const hasCountryWidget = Boolean(selectedDial || phoneContainer?.querySelector(".iti__selected-dial-code, .iti__selected-country, .iti__selected-flag, [class*='iti__'], mat-select, [role='combobox'], [aria-haspopup]"));

	      if (hasCountryWidget) {
	        return selectedDial === expectedPhone.dialCode;
	      }

	      return rawDigits.startsWith(expectedPhone.dialDigits) || rawDigits.length === expectedPhone.localDigits.length;
	    }

	    function parseExpectedPhone(expectedValue: string) {
	      ...
	      const dialDigits = expectedDigits.length > localDigits.length ? expectedDigits.slice(0, -localDigits.length) : "";

	      return {
	        localDigits,
	        dialDigits,
	        dialCode: dialDigits ? `+${dialDigits}` : ""
	      };
	    }

		    function findPhoneTextInput(targetElement: HTMLElement) {
	      if (targetElement instanceof HTMLInputElement || targetElement instanceof HTMLTextAreaElement) {
	        return targetElement;
	      }

	      const control = targetElement.querySelector("input[type='tel'], input[inputmode='tel'], input[name*='phone' i], input[id*='phone' i], input[name*='mobile' i], input[id*='mobile' i]");

		      return control instanceof HTMLInputElement ? control : undefined;
		    }

	    function normalizePhoneDial(value: string) {
	      const digits = value.replace(/\D+/g, "");
	      return digits ? `+${digits}` : "";
	    }

		    function collectPhoneDialText(targetElement: HTMLElement, container: HTMLElement | null | undefined) {
	      const phoneContainer = findPhoneContainer(targetElement) ?? container;
	      const instanceDial = getIntlTelInputDial(findPhoneTextInput(targetElement));
	      ...
	      const candidates = Array.from(phoneContainer.querySelectorAll([
	        ".iti__selected-dial-code",
	        ".selected-dial-code",
	        ".iti__selected-country",
	        ".iti__selected-flag",
	      ...
	      for (const candidate of candidates) {
	        if (!isVisible(candidate) || isInPhoneOptionList(candidate)) {
	          continue;
	        }
	    ...
	    function getIntlTelInputDial(input: HTMLInputElement | HTMLTextAreaElement | undefined) {
	      if (!(input instanceof HTMLInputElement)) {
	        return "";
	      }
	      ...
	      const dialDigits = win.intlTelInputGlobals?.getInstance?.(input)?.getSelectedCountryData?.().dialCode?.replace(/\D+/g, "");
	    ...
	    function isInPhoneOptionList(element: HTMLElement) {
	      return Boolean(element.closest("[role='listbox'], [role='menu'], .iti__country-list, .iti__country, .cdk-overlay-pane, mat-option, [class*='option']"));
	    }

	    function findPhoneContainer(targetElement: HTMLElement) {
    ...
    function hasPhoneExpectedValue(expectedValue: string) {
      return expectedValue.replace(/\D+/g, "").length >= 8;
    }

    function phoneDigitsMatch(actual: string, expectedValue: string) {
      ...
      const expectedLocal = expectedDigits.slice(-10);

      if (!actualDigits.endsWith(expectedLocal)) {
        return false;
      }
      ...
      const expectedCountry = expectedDigits.slice(0, -expectedLocal.length);

      return Boolean(expectedCountry) && actualDigits.startsWith(expectedCountry);
    }
...
function isPhoneLikeFillField(field: BrowserFillField) {
  return field.inputType === "tel"
    || field.inputType === "phone"
    || /^\+\d{1,4}$/.test(field.label.trim())
    || /\b(phone|mobile|telephone|contact number)\b/.test(normalizeKey(field.label));
}

function getTextLikeCommitKind(field: BrowserFillField): CanonicalFieldKind | FillStrategy {
  if (field.inputType === "date") {
    return "date";
  }

  if (isPhoneLikeFillField(field)) {
    return "phone";
  }
  ...
  for (const frame of page.frames()) {
    const filled = await frame.evaluate(
      ({ fieldId, fieldValue, preferredStrategy }) => {
        ...
        // Text-like controls are committed through real keyboard input in
        // commitTextLikeLocator/fillByFreshLabelTarget. Avoid DOM value
        // injection here because controlled ATS widgets can ignore it.
  ...
  for (const frame of page.frames()) {
    ...
    try {
      ...
      if (field.inputType === "select") {
        ...
        if (await commitTextLikeLocator(locator, field, getTextLikeCommitKind(field))) {
          return true;
        }
  ...
  for (const frame of page.frames()) {
    ...
    try {
      if (field.inputType === "select") {
        ...
        if (await commitTextLikeLocator(target, { ...field, value }, getTextLikeCommitKind(field))) {
          return true;
        }
      }
    } catch (_error) {
...
async function fillTextField(page: Page, field: BrowserFillField) {
  ...
  for (const frame of page.frames()) {
    const marker = `gl-text-field-real-type-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const found = await frame.evaluate(
      ({ fieldId, label, value, marker }) => {
        const searchRoots = getSearchRoots();
        ...
        best.setAttribute("data-gradlaunch-text-field-target", marker);
        return true;
      ...
      { fieldId: field.fieldId, label: normalizedLabel, value: field.value, marker }
    ).catch(() => false);

    if (found) {
      const target = frame.locator(`[data-gradlaunch-text-field-target="${marker}"]`).first();

      try {
        if (await commitTextLikeLocator(target, field, getTextLikeCommitKind(field))) {
          return true;
        }
      } finally {
        await target.evaluate((element) => {
          if (element instanceof HTMLElement) {
            element.removeAttribute("data-gradlaunch-text-field-target");
          }
        }).catch(() => undefined);
      }
    }
...
async function fillSelectLikeField(page: Page, field: BrowserFillField) {
  ...
  const isLocationSelectLike = isLocationLikeDescriptor(normalizeInline(`${field.label} ${value}`));
  const useAutocompleteTyping = isAutocomplete || isLocationSelectLike;
  const queries = getSelectLikeQueries(field.label, value);

  if (isAutocomplete && !isLocationSelectLike && await fillSearchAutocompleteByDom(page, field, queries)) {
    return true;
  }
  ...
  for (const frame of page.frames()) {
    ...
    for (const target of targets) {
      try {
        ...
        const interactionTarget = await getSelectInteractionTarget(target!);
        const comboboxLikeTarget = await isComboboxLikeTarget(interactionTarget);

        if (await verifySelectLikeValue(target!, value, { allowPartial: !useAutocompleteTyping, fieldLabel: field.label })
          || await verifySelectLikeValue(interactionTarget, value, { allowPartial: !useAutocompleteTyping, fieldLabel: field.label })) {
          return true;
        }
        ...
        for (const query of queries) {
          ...
          await clearAndTypeSelectLikeQuery(interactionTarget, query, useAutocompleteTyping || comboboxLikeTarget);

          let dropdownScope = await waitForLegacyDropdownScope(
            frame,
            interactionTarget,
            useAutocompleteTyping ? Number(process.env.BROWSER_LOCATION_MATCH_TIMEOUT_MS ?? 2600) : 700
          );

          try {
            if (isLocationSelectLike) {
              if (dropdownScope) {
                await waitForLegacyMatchingLocationOption(dropdownScope, value, query);
              }

              if (await clickLocationSuggestionByText(frame, value, field.label, query, dropdownScope)) {
                await interactionTarget.press("Tab", { timeout: 350 }).catch(() => undefined);
                await interactionTarget.blur().catch(() => undefined);
                await page.waitForTimeout(320).catch(() => undefined);

                if (await hasCommittedLegacyLocationValue(page, field) || await verifyFreshSelectLikeField(page, field, "autocomplete")) {
                  return true;
                }
              }

              if (dropdownScope && await commitLegacyComboboxSuggestionByKeyboard(page, dropdownScope, value, interactionTarget)) {
                await interactionTarget.press("Tab", { timeout: 350 }).catch(() => undefined);
                await interactionTarget.blur().catch(() => undefined);
                await page.waitForTimeout(320).catch(() => undefined);
              ...
              if (dropdownScope && await commitLegacyDropdownByKeyboard(page, dropdownScope)) {
                await interactionTarget.press("Tab", { timeout: 350 }).catch(() => undefined);
                await interactionTarget.blur().catch(() => undefined);
                await page.waitForTimeout(320).catch(() => undefined);
            ...
            if (dropdownScope) {
              await cleanupLegacyDropdownScope(dropdownScope);
            }
          }
        ...
        await interactionTarget.press("Enter", { timeout: 400 }).catch(() => undefined);
        const verified = await verifySelectLikeValue(target!, value, { allowPartial: !useAutocompleteTyping, fieldLabel: field.label, acceptLocationTextValue: true })
          || await verifySelectLikeValue(interactionTarget, value, { allowPartial: !useAutocompleteTyping, fieldLabel: field.label, acceptLocationTextValue: true });
...
async function isComboboxLikeTarget(target: Locator) {
  return target.evaluate((element) => {
    ...
    return element.getAttribute("role") === "combobox"
      ...
      || Boolean(element.getAttribute("aria-owns"))
      || element.getAttribute("aria-haspopup") === "listbox";
  }).catch(() => false);
}

async function verifyFreshSelectLikeField(page: Page, field: BrowserFillField, kind: CanonicalFieldKind | FillStrategy) {
  const value = field.value.trim();
  const aliases = getFieldAliases(field.label);
  ...
  for (const frame of page.frames()) {
    ...
    for (const target of targets) {
      ...
      if (await verifySelectLikeValue(target, value, { allowPartial: false, fieldLabel: field.label, acceptLocationTextValue: true })
        || await verifySelectLikeValue(interactionTarget, value, { allowPartial: false, fieldLabel: field.label, acceptLocationTextValue: true })
        || await verifyLocatorCommitted(target, field, { kind, allowPartial: false, acceptLocationTextValue: true })
        || await verifyLocatorCommitted(interactionTarget, field, { kind, allowPartial: false, acceptLocationTextValue: true })) {
        return true;
      }
    }
  }
...
async function hasCommittedLegacyLocationValue(page: Page, field: BrowserFillField) {
  const debug = await collectLegacyFillFieldDebug(page, field);
  const actual = typeof debug === "object" && debug && "control" in debug
    ? (debug as { control?: { value?: string; ariaExpanded?: string } }).control
    : undefined;
  const actualValue = actual?.value?.trim() ?? "";

  if (!actualValue) {
    return false;
  }

  return legacyHighlightedOptionMatches(actualValue, field.value)
    && actual?.ariaExpanded !== "true";
}

async function clearAndTypeSelectLikeQuery(target: Locator, query: string, humanLike: boolean) {
  const selectAll = process.platform === "darwin" ? "Meta+A" : "Control+A";
  await target.press(selectAll, { timeout: 250 }).catch(() => undefined);
  await target.press("Backspace", { timeout: 250 }).catch(() => undefined);

  if (humanLike) {
    const seeded = await seedSelectLikeQuery(target, query);

    if (seeded && await locatorValueContains(target, query)) {
      await target.page().waitForTimeout(180).catch(() => undefined);
      return;
    }
  }

  await target.type(query, {
    delay: humanLike ? Number(process.env.BROWSER_REAL_TYPE_DELAY_MS ?? 28) : 10,
    timeout: Math.max(humanLike ? 1800 : 900, query.length * (humanLike ? 110 : 60))
  }).catch(() => undefined);
  await target.page().waitForTimeout(humanLike ? 180 : 120).catch(() => undefined);
}

async function seedSelectLikeQuery(target: Locator, query: string) {
  return target.evaluate((element, nextValue) => {
    if (!(element instanceof HTMLElement)) {
      return false;
    }
...
async function locatorValueContains(target: Locator, query: string) {
  return target.evaluate((element, expected) => {
    if (!(element instanceof HTMLElement)) {
      return false;
    }
...
async function waitForLegacyDropdownScope(frame: Frame, locator: Locator, timeoutMs = 2200): Promise<LegacyDropdownScope | undefined> {
  const startedAt = Date.now();

  while (Date.now() - startedAt < timeoutMs) {
    const scope = await markLegacyDropdownPopup(frame, locator);

    if (scope && await legacyDropdownHasViableOptions(scope)) {
      return scope;
    }

    if (scope) {
      await cleanupLegacyDropdownScope(scope);
    }

    await frame.page().waitForTimeout(90).catch(() => undefined);
  }

  return markLegacyDropdownPopup(frame, locator);
}

async function markLegacyDropdownPopup(frame: Frame, locator: Locator): Promise<LegacyDropdownScope | undefined> {
  const popupMarker = `gl-legacy-dropdown-popup-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const found = await locator.evaluate((control, popupMarker) => {
    ...
    popup.setAttribute("data-gradlaunch-legacy-dropdown-popup", popupMarker);
    return true;
...
async function legacyDropdownHasViableOptions(scope: LegacyDropdownScope) {
...
async function waitForLegacyMatchingLocationOption(scope: LegacyDropdownScope | undefined, expectedValue: string, typedQuery?: string) {
  if (!scope) {
    return false;
  }
  ...
  while (Date.now() - startedAt < timeoutMs) {
    const options = await readLegacyVisibleOptionTexts(scope);

    if (options.some((option) => legacyHighlightedOptionMatches(option, expectedValue))
      || (typedQuery && options.some((option) => legacyHighlightedOptionMatches(option, typedQuery)))) {
      return true;
    }
...
async function commitLegacyDropdownByKeyboard(page: Page, scope?: LegacyDropdownScope) {
  if (scope && !await legacyDropdownHasViableOptions(scope)) {
    return false;
  }
...
async function commitLegacyComboboxSuggestionByKeyboard(page: Page, scope?: LegacyDropdownScope, expectedValue?: string, focusTarget?: Locator) {
  if (!scope || !await legacyDropdownHasViableOptions(scope)) {
    return false;
  }
  ...
  for (let move = 0; move < maxMoves; move += 1) {
    const highlighted = await readLegacyHighlightedOption(scope);

    if (highlighted && (!expectedValue || legacyHighlightedOptionMatches(highlighted, expectedValue))) {
      await page.keyboard.press("Enter").catch(() => undefined);
...
async function readLegacyVisibleOptionTexts(scope: LegacyDropdownScope) {
  return scope.frame.evaluate((popupMarker) => {
    const popup = findPopupByMarker(popupMarker);
    ...
    return Array.from(popup.querySelectorAll([
      ...
      .map((element) => `${element.innerText || element.textContent || ""} ${element.getAttribute("data-value") ?? ""}`.replace(/\s+/g, " ").trim())
...
async function readLegacyHighlightedOption(scope: LegacyDropdownScope) {
  return scope.frame.evaluate((popupMarker) => {
    const popup = findPopupByMarker(popupMarker);
...
async function cleanupLegacyDropdownScope(scope: LegacyDropdownScope) {
  await scope.frame.evaluate((popupMarker) => {
    for (const root of getSearchRoots()) {
      root
        .querySelector(`[data-gradlaunch-legacy-dropdown-popup="${CSS.escape(popupMarker)}"]`)
        ?.removeAttribute("data-gradlaunch-legacy-dropdown-popup");
    }
...
function legacyHighlightedOptionMatches(actual: string, expected: string) {
  const actualKey = normalizeInline(actual);
  const expectedKey = normalizeInline(expected);
  ...
  const aliases = new Set<string>([expectedKey]);

  if (/\bgurugram|gurgaon\b/.test(expectedKey)) {
    aliases.add("gurugram");
    aliases.add("gurgaon");
  }

  if (/\bbengaluru|bangalore|banglore\b/.test(expectedKey)) {
    aliases.add("bengaluru");
    aliases.add("bangalore");
    aliases.add("banglore");
  }
  ...
  for (const alias of aliases) {
    if (alias && (actualKey === alias || actualKey.includes(alias) || alias.includes(actualKey))) {
      return true;
    }
  }
...
async function fillSplLocationAutocomplete(page: Page, field: BrowserFillField): Promise<boolean | undefined> {
  ...
  for (const frame of page.frames()) {
    const marker = `gl-spl-location-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const found = await markSplLocationAutocomplete(frame, field, marker);

    if (!found) {
      continue;
    }

    const widget = frame.locator(`[data-gradlaunch-spl-location="${marker}"]`).first();

    try {
      await widget.scrollIntoViewIfNeeded().catch(() => undefined);
      await widget.click({ force: true, timeout: 900 }).catch(() => undefined);
      await page.waitForTimeout(140).catch(() => undefined);

      const query = getLocationQueryCity(expected);
      const typingTarget = await markSplLocationTypingTarget(frame, marker);
      let typed = false;

      if (typingTarget) {
        typed = await typeIntoSplLocationTarget(frame, marker, query);
      } else {
        const seeded = await seedSplLocationQuery(frame, marker, query);

        if (!seeded) {
          await page.keyboard.insertText(query).catch(() => undefined);
          await page.keyboard.type(query, { delay: 28 }).catch(() => undefined);
        }

        typed = seeded || await verifySplLocationQuery(frame, marker, query);
      }
      ...
      if (!await waitAndClickSplLocationOption(frame, expected, query)) {
        return false;
      }

      await page.waitForTimeout(420).catch(() => undefined);
      return await verifySplLocationAutocomplete(frame, marker, expected, query);
    } finally {
      await cleanupSplLocationAutocomplete(frame, marker);
    }
  }
...
async function markSplLocationAutocomplete(frame: Frame, field: BrowserFillField, marker: string) {
  return frame.evaluate(({ fieldId, label, value, marker }) => {
    const labelKey = normalize(label);
    const valueKey = normalize(value);
    const widgets = Array.from(document.querySelectorAll([
      "spl-autocomplete[data-test='location-autocomplete']",
      "spl-autocomplete[data-sr-id='location-autocomplete-search']",
      "[data-test='personal-info-location'] spl-autocomplete",
    ...
    for (const widget of widgets) {
      ...
      const descriptor = normalize([
        widget.getAttribute("label"),
        widget.getAttribute("data-test"),
        widget.getAttribute("data-sr-id"),
        widget.id,
        widget.textContent,
        widget.closest("[data-test]")?.getAttribute("data-test"),
        widget.closest("oc-location-autocomplete-wrapper, oc-location-autocomplete")?.textContent
      ].filter(Boolean).join(" "));
      let score = 0;

      if (fieldId && widget.getAttribute("data-gradlaunch-field-id") === fieldId) score += 220;
      if (/\blocation autocomplete|personal info location\b/.test(descriptor)) score += 130;
      if (/\bcity|location|place of residence|residence\b/.test(descriptor)) score += 100;
      if (labelKey && (descriptor.includes(labelKey) || labelKey.includes(descriptor))) score += 70;
      if (/\bgurugram|gurgaon|bengaluru|bangalore|bhiwani|aurangabad|india\b/.test(valueKey)) score += 40;
    ...
    best.setAttribute("data-gradlaunch-spl-location", marker);
    return true;
  ...
  }, { fieldId: field.fieldId, label: field.label, value: field.value, marker }).catch(() => false);
}

async function markSplLocationTypingTarget(frame: Frame, marker: string) {
  return frame.evaluate((marker) => {
    const widget = document.querySelector(`[data-gradlaunch-spl-location="${CSS.escape(marker)}"]`) as HTMLElement | null;

    if (!widget) {
      return false;
    }

    const initialTarget = findTypingTarget(widget)
      ?? findTypingTarget(widget.shadowRoot);

    if (initialTarget) {
      focusTypingTarget(initialTarget);
    } else {
      activateWidget(widget);
    }

    const target = findTypingTarget(widget)
      ?? findTypingTarget(widget.shadowRoot)
      ?? deepActiveTypingTarget(document);

    if (!target) {
      return false;
    }

    target.setAttribute("data-gradlaunch-spl-location-input", marker);
    return true;

    function deepActiveTypingTarget(root: Document | ShadowRoot): HTMLElement | undefined {
      const active = root.activeElement;
      ...
      if (active.shadowRoot) {
        return deepActiveTypingTarget(active.shadowRoot);
      }

      return undefined;
    }

    function findTypingTarget(root: ParentNode | null | undefined): HTMLElement | undefined {
      if (!root) {
        return undefined;
      }

      const direct = Array.from(root.querySelectorAll("input:not([type='hidden']), textarea, [contenteditable='true'], [role='textbox'], [role='combobox']"))
        .find((element): element is HTMLElement => element instanceof HTMLElement && isTypingTarget(element));
      ...
      for (const element of Array.from(root.querySelectorAll("*")) as HTMLElement[]) {
        const nested = findTypingTarget(element.shadowRoot);
    ...
    function activateWidget(widget: HTMLElement) {
      const activators = [
        findActivator(widget),
        findActivator(widget.shadowRoot),
        widget
      ].filter((element): element is HTMLElement => element instanceof HTMLElement);
    ...
    function findActivator(root: ParentNode | null | undefined): HTMLElement | undefined {
      if (!root) {
        return undefined;
      }
      ...
      for (const element of Array.from(root.querySelectorAll("*")) as HTMLElement[]) {
        const nested = findActivator(element.shadowRoot);
    ...
    function focusTypingTarget(element: HTMLElement) {
    ...
    function isTypingTarget(element: HTMLElement) {
      if (element instanceof HTMLInputElement) {
        return !element.disabled && !["hidden", "file", "checkbox", "radio", "submit", "button", "image", "reset"].includes(element.type);
      }
      ...
      return element.isContentEditable || element.getAttribute("role") === "textbox" || element.getAttribute("role") === "combobox";
    }
...
async function typeIntoSplLocationTarget(frame: Frame, marker: string, query: string) {
  const target = frame.locator(`[data-gradlaunch-spl-location-input="${marker}"]`).first();
  const selectAll = process.platform === "darwin" ? "Meta+A" : "Control+A";

  await target.click({ force: true, timeout: 900 }).catch(() => undefined);
  await target.press(selectAll, { timeout: 350 }).catch(() => undefined);
  await target.press("Backspace", { timeout: 350 }).catch(() => undefined);
  await target.type(query, { delay: 28, timeout: Math.max(1800, query.length * 110) }).catch(() => undefined);

  if (await verifySplTypingTargetValue(frame, marker, query)) {
    return true;
  }
  ...
  return verifySplTypingTargetValue(frame, marker, query);
}

async function verifySplTypingTargetValue(frame: Frame, marker: string, query: string) {
  return frame.evaluate(({ marker, query }) => {
    const target = document.querySelector(`[data-gradlaunch-spl-location-input="${CSS.escape(marker)}"]`) as HTMLElement | null;
    ...
    return normalize(actual).includes(normalize(query));

    function normalize(raw: string | null | undefined) {
      return (raw ?? "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
    }
  }, { marker, query }).catch(() => false);
}

async function verifySplLocationQuery(frame: Frame, marker: string, query: string) {
  return frame.evaluate(({ marker, query }) => {
    const widget = document.querySelector(`[data-gradlaunch-spl-location="${CSS.escape(marker)}"]`) as HTMLElement | null;
    ...
    const target = document.querySelector(`[data-gradlaunch-spl-location-input="${CSS.escape(marker)}"]`) as HTMLElement | null;
    const active = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    const widgetValue = normalize([
      widget.getAttribute("value"),
      widget.getAttribute("aria-valuetext"),
      widget.textContent
    ].filter(Boolean).join(" "));
    const targetValue = target instanceof HTMLInputElement || target instanceof HTMLTextAreaElement
      ? normalize(target.value)
      : target?.isContentEditable
        ? normalize(target.textContent)
        : "";
    const activeValue = active instanceof HTMLInputElement || active instanceof HTMLTextAreaElement
    ...
    return Boolean(
      queryKey
      && (
        targetValue.includes(queryKey)
        || activeValue.includes(queryKey)
        || widgetValue.includes(queryKey)
      )
    );
...
async function seedSplLocationQuery(frame: Frame, marker: string, query: string) {
  return frame.evaluate(({ marker, query }) => {
    const widget = document.querySelector(`[data-gradlaunch-spl-location="${CSS.escape(marker)}"]`) as HTMLElement | null;
    ...
    if (target instanceof HTMLInputElement || target instanceof HTMLTextAreaElement) {
      ...
      descriptor?.set?.call(target, query);
      target.dispatchEvent(new InputEvent("input", { bubbles: true, composed: true, data: query, inputType: "insertText" }));
    ...
    const anyWidget = widget as HTMLElement & { value?: unknown };

    if ("value" in anyWidget) {
      anyWidget.value = query;
    }

    widget.setAttribute("value", query);
    widget.dispatchEvent(new InputEvent("input", { bubbles: true, composed: true, data: query, inputType: "insertText" }));
    widget.dispatchEvent(new Event("change", { bubbles: true }));
    widget.focus?.();
    return true;

    function activeTypingTarget(root: Document | ShadowRoot): HTMLElement | undefined {
      const active = root.activeElement;
      ...
      if (active.shadowRoot) {
        return activeTypingTarget(active.shadowRoot);
      }
...
async function waitAndClickSplLocationOption(frame: Frame, expected: string, query: string) {
  const timeoutMs = Number(process.env.BROWSER_LOCATION_OPTION_TIMEOUT_MS ?? 2200);
  const startedAt = Date.now();

  while (Date.now() - startedAt < timeoutMs) {
    if (await clickSplLocationOption(frame, expected, query)) {
      return true;
    }
...
async function clickSplLocationOption(frame: Frame, expected: string, query: string) {
  const marker = `gl-spl-location-option-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const found = await frame.evaluate(({ expected, query, marker }) => {
    const expectedKey = normalize(expected);
    const queryKey = normalize(query);
    const expectedCountry = inferCountry(expectedKey) ?? inferCountry(queryKey);
    ...
    for (const candidate of candidates) {
      ...
      const rawText = (candidate.innerText || candidate.textContent || "").replace(/\s+/g, " ").trim();
      const text = normalize(`${rawText} ${candidate.getAttribute("data-value") ?? ""}`);

      if (!text || text.length > 180 || /\b(cannot find|fill in manually|manually|please provide|no matches|no results)\b/.test(text)) {
        continue;
      }

      let score = 0;

      if (expectedKey && (text === expectedKey || text.includes(expectedKey) || expectedKey.includes(text))) {
        score += 130;
      }

      for (const alias of aliases) {
        if (alias.length > 2 && hasPhrase(text, alias)) {
          score += text.startsWith(alias) ? 120 : 90;
          break;
        }
      }
      ...
      if (expectedCountry && mentionsDifferentCountry(text, expectedCountry)) {
        score -= 120;
      }
      ...
      if (candidate.closest("[role='listbox'], [role='menu'], [class*='dropdown' i], [class*='menu' i], [class*='option' i], [class*='suggest' i], [class*='result' i], [class*='popover' i], [class*='list' i]")) {
        score += 18;
      }

      const rect = candidate.getBoundingClientRect();
      score -= Math.min(Math.max(0, rect.height - 70) / 3, 35);
      score -= Math.min(Math.max(0, text.length - 80) / 5, 30);
    ...
    target.setAttribute("data-gradlaunch-spl-location-option", marker);
    return true;

    function clickableTarget(element: HTMLElement) {
      return element.closest("[role='option'], mat-option, li, button, [data-value], [class*='option' i]") as HTMLElement | null ?? element;
    }
    ...
    function mentionsDifferentCountry(text: string, desiredCountry: string) {
      return ["india", "australia", "canada", "united states", "usa", "united kingdom", "uk"]
        .some((country) => country !== desiredCountry && hasPhrase(text, country));
    }

    function hasPhrase(text: string, phrase: string) {
      const normalizedPhrase = normalize(phrase);
      return Boolean(text && normalizedPhrase && new RegExp(`(^| )${escapeRegExp(normalizedPhrase)}( |$)`).test(text));
    }
  ...
  }, { expected, query, marker }).catch(() => false);

  if (!found) {
    return false;
  }

  const option = frame.locator(`[data-gradlaunch-spl-location-option="${marker}"]`).first();
  ...
  try {
    ...
    if (box) {
      ...
      await frame.page().mouse.move(clickX, clickY).catch(() => undefined);
      await frame.page().mouse.down().catch(() => undefined);
      await frame.page().waitForTimeout(45).catch(() => undefined);
      await frame.page().mouse.up().catch(() => undefined);
    } else {
      await option.click({ force: true, timeout: 900 }).catch(() => undefined);
    }

    await frame.page().waitForTimeout(260).catch(() => undefined);
    return true;
  } finally {
    await option.evaluate((element) => {
      if (element instanceof HTMLElement) {
        element.removeAttribute("data-gradlaunch-spl-location-option");
      }
    }).catch(() => undefined);
  }
}

async function verifySplLocationAutocomplete(frame: Frame, marker: string, expected: string, query: string) {
  return frame.evaluate(({ marker, expected, query }) => {
    const widget = document.querySelector(`[data-gradlaunch-spl-location="${CSS.escape(marker)}"]`) as HTMLElement | null;
    ...
    const queryKey = normalize(query);
    const aliases = getLocationAliases(expectedKey, queryKey);
    const attrValue = normalize(widget.getAttribute("value"));
    const text = normalize(widget.textContent);
    const container = widget.closest("[data-test='personal-info-location'], oc-location-autocomplete-wrapper, oc-location-autocomplete") as HTMLElement | null;
    const containerText = normalize(container?.textContent);
    const invalid = widget.getAttribute("aria-invalid") === "true"
      || widget.classList.contains("ng-invalid")
      || container?.classList.contains("ng-invalid")
      || Boolean(container?.querySelector("[aria-invalid='true'], .ng-invalid"));

    if (invalid) {
      return false;
    }

    if (attrValue && attrValue !== queryKey && attrValue !== expectedKey) {
      return true;
    }

    return aliases.some((alias) => alias.length > 2 && (hasPhrase(text, alias) || hasPhrase(containerText, alias)))
      || Boolean(expectedKey && (text.includes(expectedKey) || containerText.includes(expectedKey)));
  ...
  }, { marker, expected, query }).catch(() => false);
}

async function cleanupSplLocationAutocomplete(frame: Frame, marker: string) {
  await frame.evaluate((marker) => {
    document.querySelector(`[data-gradlaunch-spl-location="${CSS.escape(marker)}"]`)?.removeAttribute("data-gradlaunch-spl-location");
    document.querySelector(`[data-gradlaunch-spl-location-input="${CSS.escape(marker)}"]`)?.removeAttribute("data-gradlaunch-spl-location-input");
    document.querySelector(`[data-gradlaunch-spl-location-option="${CSS.escape(marker)}"]`)?.removeAttribute("data-gradlaunch-spl-location-option");
  }, marker).catch(() => undefined);
}

async function fillSearchAutocompleteByDom(page: Page, field: BrowserFillField, queries: string[]) {
  const expected = field.value.trim();
  const isLocationAutocomplete = isLocationLikeDescriptor(normalizeInline(`${field.label} ${expected}`));
  ...
  for (const frame of page.frames()) {
    ...
    for (const query of queries) {
      ...
      await target.click({ force: true, timeout: 900 }).catch(() => undefined);
      await clearAutocompleteTarget(target);
      await target.type(cleanQuery, { delay: 35, timeout: 1800 }).catch(() => undefined);

      const clicked = isLocationAutocomplete
        ? await clickLocationSuggestionByText(frame, expected, field.label, cleanQuery)
        : await clickBestAutocompleteSuggestion(frame, marker, expected, field.label, cleanQuery);
...
async function clickLocationSuggestionByText(
  ...
  fieldLabel: string,
  query: string,
  scope?: LegacyDropdownScope
) {
  const marker = `gl-location-suggestion-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const found = await frame.evaluate(({ expected, fieldLabel, query, marker, popupMarker }) => {
    const expectedKey = normalize(expected);
    const queryKey = normalize(query);
    const isLocationLike = /\b(location|city|place|residence|country|state|province|region|india|australia|canada|united states|usa|united kingdom|uk|haryana|bihar|maharashtra|karnataka|uttar pradesh|telangana|tamil nadu|west bengal|bhiwani|gurugram|gurgaon|aurangabad|bengaluru|bangalore|delhi|noida|hyderabad|pune|mumbai|chennai|kolkata)\b/.test(normalize(`${fieldLabel} ${expected}`));
    ...
    target.setAttribute("data-gradlaunch-location-suggestion", marker);
    return true;
  ...
  }, { expected: value, fieldLabel, query, marker, popupMarker: scope?.popupMarker }).catch(() => false);
  ...
  try {
    const clicked = await frame.evaluate((optionMarker) => {
      ...
      function findOptionByMarker(marker: string) {
        for (const root of getSearchRoots()) {
          const match = root.querySelector(`[data-gradlaunch-location-suggestion="${CSS.escape(marker)}"]`);
    ...
    await frame.evaluate((optionMarker) => {
      for (const root of getSearchRoots()) {
        root
          .querySelector(`[data-gradlaunch-location-suggestion="${CSS.escape(optionMarker)}"]`)
          ?.removeAttribute("data-gradlaunch-location-suggestion");
      }
...
async function fillCountrySelectField(page: Page, field: BrowserFillField) {
  ...
  const desiredDialDigits = countryDialDigitsFromValue(field.value);

  if (!desiredCountry && !desiredDialDigits) {
    return false;
  }
  ...
  for (const frame of page.frames()) {
    ...
    for (const target of targets) {
      ...
      }, desiredCountry ?? field.value).catch(() => false);

      if (filledNative && await verifySelectLikeValue(target, desiredCountry ?? field.value, { allowPartial: false, fieldLabel: field.label })) {
        return true;
      }
      ...
      if (desiredCountry && await clickExactCountryOption(frame, desiredCountry)) {
        await target.press("Tab", { timeout: 350 }).catch(() => undefined);
        await page.waitForTimeout(120).catch(() => undefined);

        if (await verifySelectLikeValue(target, desiredCountry, { allowPartial: false, fieldLabel: field.label })) {
          return true;
        }
      }

      if (desiredDialDigits && await clickCountryCodeOptionByDial(frame, desiredCountry ?? field.value, desiredDialDigits)) {
        await target.press("Tab", { timeout: 350 }).catch(() => undefined);
        await page.waitForTimeout(120).catch(() => undefined);

        if (await verifySelectLikeValue(target, desiredCountry ?? field.value, { allowPartial: false, fieldLabel: field.label })) {
          return true;
        }
      }
      ...
      await target.press("Backspace", { timeout: 250 }).catch(() => undefined);
      await target.type(desiredCountry ?? field.value, { delay: 12, timeout: 1000 }).catch(() => undefined);
      await page.waitForTimeout(150).catch(() => undefined);
  ...
  return target.evaluate((element, { wantedValue, allowPartial, fieldLabel, acceptLocationTextValue }) => {
    ...
    function matchesExpected(actualValue: string, matchOptions?: { allowLocationAlias?: boolean }) {
      ...
      if (/\bcountry\b/.test(descriptor)) {
        ...
        const desiredDial = inferDialCode(wanted);

        if (/\b(country code|country region code|search by country region or code|dial code|phone code)\b/.test(descriptor) && desiredDial) {
          const actualDigits = actualValue.replace(/\D+/g, "");
          const wantedDigits = desiredDial.replace(/\D+/g, "");

          if (actualDigits === wantedDigits || actualDigits.startsWith(wantedDigits) || hasPhrase(actualValue, wantedDigits) || hasPhrase(actualValue, desiredDial)) {
            return true;
          }
        }
    ...
    function inferDialCode(value: string) {
      if (hasPhrase(value, "india")) {
        return "+91";
      }
      ...
      if (hasPhrase(value, "united states") || hasPhrase(value, "usa") || value === "us" || hasPhrase(value, "canada")) {
        return "+1";
      }
...
function countryDialDigitsFromValue(value: string) {
  const country = extractDesiredCountries(value)[0];
...
async function clickCountryCodeOptionByDial(frame: Frame, desiredCountry: string, desiredDialDigits: string) {
  const marker = `gl-country-code-option-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const found = await frame.evaluate(({ desiredCountry, desiredDialDigits, marker }) => {
    const wantedCountry = normalize(desiredCountry);
    const wantedDial = desiredDialDigits.replace(/\D+/g, "");
    const candidates = Array.from(document.querySelectorAll([
      "[data-dial-code]",
      ".iti__country",
      "[role='option']",
    ...
    for (const candidate of candidates) {
      ...
      const dial = (candidate.getAttribute("data-dial-code") ?? "").replace(/\D+/g, "");
      ...
      if (dial && dial === wantedDial) {
        score += 220;
      }

      if (wantedDial && (text.includes(` ${wantedDial} `) || text.endsWith(` ${wantedDial}`) || text.includes(`+${wantedDial}`))) {
        score += 160;
      }

      if (wantedCountry && hasPhrase(text, wantedCountry)) {
        score += 120;
      }

      if (candidate.closest("[role='listbox'], [role='menu'], [role='dialog'], .cdk-overlay-pane, [class*='dropdown'], [class*='menu'], [class*='listbox'], [class*='option']")) {
        score += 30;
      }
    ...
    best.setAttribute("data-gradlaunch-country-code-option-target", marker);
    return true;
  ...
  }, { desiredCountry, desiredDialDigits, marker }).catch(() => false);
  ...
  try {
    const clicked = await frame.evaluate((optionMarker) => {
      const option = document.querySelector(`[data-gradlaunch-country-code-option-target="${CSS.escape(optionMarker)}"]`);
    ...
    await frame.evaluate((optionMarker) => {
      document
        .querySelector(`[data-gradlaunch-country-code-option-target="${CSS.escape(optionMarker)}"]`)
        ?.removeAttribute("data-gradlaunch-country-code-option-target");
    }, marker).catch(() => undefined);
  }
}
...
async function clickVisibleSelectOption(
  ...
  const found = await frame.evaluate(({ expected, fieldLabel, query, marker }) => {
    ...
    const candidates = getSearchRoots().flatMap((root) => Array.from(root.querySelectorAll([
      "[role='option']",
      "[role='menuitemradio']",
      "[role='menuitemcheckbox']",
      "[role='radio']",
  ...
  try {
    return await frame.evaluate((optionMarker) => {
      const option = findOptionByMarker(optionMarker);
      ...
      function findOptionByMarker(marker: string) {
        for (const root of getSearchRoots()) {
          const match = root.querySelector(`[data-gradlaunch-option-target="${CSS.escape(marker)}"]`);
    ...
    await frame.evaluate((optionMarker) => {
      for (const root of getSearchRoots()) {
        root
          .querySelector(`[data-gradlaunch-option-target="${CSS.escape(optionMarker)}"]`)
          ?.removeAttribute("data-gradlaunch-option-target");
      }
...
function isSplLocationAutocompleteField(field: BrowserFillField) {
  const label = normalizeInline(field.label);
  const descriptor = normalizeInline(`${field.label} ${field.value}`);

  return /\b(city|location|place of residence|residence)\b/.test(label)
    && isLocationLikeDescriptor(descriptor);
}
