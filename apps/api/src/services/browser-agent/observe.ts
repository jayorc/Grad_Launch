...
export async function observeBrowserPage(
  ...
  const validationMessages = input?.validationMessages ?? await getVisibleValidationMessages(page);
  const domObservations = await Promise.all(page.frames().map(async (frame, frameIndex) => {
    return frame.evaluate((currentFrameIndex) => {
      const searchRoots = getSearchRoots();
      ...
      for (const [index, element] of candidates.entries()) {
        ...
        const id = `gl-agent-control-f${currentFrameIndex}-${index}`;
    ...
    }, frameIndex).catch(() => ({ title: "", pageText: "", controls: [] as ObservedControl[] }));
  }));

  const domObservation = {
    title: domObservations.map((item) => item.title).find((value) => value.trim()) ?? "",
    pageText: domObservations
      .map((item) => item.pageText)
      .filter(Boolean)
      .join(" ")
      .replace(/\s+/g, " ")
      .slice(0, 3500),
    controls: domObservations.flatMap((item) => item.controls)
  };
...
export async function collectStageReadinessSignals(page: Page): Promise<StageReadinessSignals> {
  const [outstandingRequired, validationMessages, uploadVisible, submitVisible] = await Promise.all([
    getVisibleRequiredEmptyLabels(page).catch(() => []),
    getVisibleValidationMessages(page).catch(() => []),
    hasFileUpload(page).catch(() => false),
    hasFinalSubmitControl(page).catch(() => false)
  ]);
...
export async function collectStageSnapshot(page: Page, seedVisibleFields?: VisibleField[]): Promise<StagePageSnapshot> {
  const visibleFields = seedVisibleFields ?? await discoverVisibleFields(page);
  const readiness = await collectStageReadinessSignals(page);
  const observation = await observeBrowserPage(page, visibleFields, {
    validationMessages: readiness.validationMessages
  });

  return {
    visibleFields,
    observation,
    outstandingRequired: readiness.outstandingRequired,
    validationMessages: readiness.validationMessages,
    uploadVisible: readiness.uploadVisible,
    submitVisible: readiness.submitVisible
  };
}
  ...
  for (const frame of page.frames()) {
    const frameLabels = await frame.evaluate(() => {
      ...
      for (const errorLabel of findRequiredErrorLabels(searchRoots)) {
        ...
        const normalizedErrorLabel = normalize(errorLabel);
        const existingSatisfied = Array.from(groups.values()).some((group) => normalize(group.label) === normalizedErrorLabel && group.satisfied);
      ...
      function findRequiredErrorLabels(roots: Array<Document | ShadowRoot>) {
        ...
        for (const element of errorElements) {
          ...
          if (control && isSatisfied(control, roots)) {
            continue;
          }
...
export async function hasFinalSubmitControl(page: Page) {
  ...
  return page.evaluate(() => {
    const controls = getSearchRoots()
      .flatMap((root) => Array.from(root.querySelectorAll("button, input[type='submit'], input[type='button'], a")))
      .filter((control): control is HTMLButtonElement | HTMLInputElement | HTMLAnchorElement => {
        return control instanceof HTMLElement && !control.closest("#gradlaunch-live-bot");
      });
...
export async function waitForStageTransition(
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (didStageSignatureChange(signatureBefore, signatureAfter)) {
...
export async function clickFinalSubmit(page: Page) {
  ...
  return page.evaluate(() => {
    const controls = (Array.from(document.querySelectorAll("button, input[type='submit'], input[type='button']")) as Array<HTMLButtonElement | HTMLInputElement>)
      .filter((control) => !control.closest("#gradlaunch-live-bot"));
...
export async function clickNextStageControl(context: BrowserContext, page: Page, options: { allowApplyStart: boolean }) {
  ...
  const fallbackCandidates = await detectFallbackNavigationControls(page, observation, options);

  for (const candidate of fallbackCandidates) {
    const clicked = await clickFallbackNavigationControl(context, page, candidate.attributeValue);
...
async function detectFallbackNavigationControls(
  ...
  return page.evaluate(({ allowApplyStart, adapterId }: { allowApplyStart: boolean; adapterId?: string }) => {
    const candidates: Array<{ attributeValue: string; score: number }> = [];
    const seen = new Set<string>();

    for (const root of getSearchRoots()) {
      const controls = Array.from(root.querySelectorAll("button, a, input[type='button'], input[type='submit'], [role='button'], [role='link']")) as HTMLElement[];
      ...
      for (const [index, control] of controls.entries()) {
        ...
        if ((control instanceof HTMLButtonElement || control instanceof HTMLInputElement) && control.disabled) {
          continue;
        }
        ...
        if (!matchesNavigationText(text, allowApplyStart)) {
          continue;
        }

        if (isRejectedNavigationText(text, allowApplyStart)) {
          continue;
        }

        seen.add(text);
        const attributeValue = `gl-agent-fallback-nav-${candidates.length}-${index}`;
        control.setAttribute("data-gradlaunch-fallback-nav-id", attributeValue);

        let score = 0;

        if (/\b(next|save and continue|save continue|continue|proceed)\b/.test(text)) {
          score += 100;
        }
        ...
        if (allowApplyStart && /\b(apply|apply now|apply for this job|apply for this position|start application|i m interested|im interested)\b/.test(text)) {
          score += 65;
        }
        ...
        if (control.tagName.toLowerCase() === "button" || control.getAttribute("role") === "button") {
          score += 8;
        }
        ...
        candidates.push({ attributeValue, score });
      }
    }
    ...
    function matchesNavigationText(text: string, allowApplyStart: boolean) {
      return /\b(next|continue|save and continue|save continue|proceed|review|review application|continue to review)\b/.test(text)
        || (allowApplyStart && /\b(apply|apply now|apply for this job|apply for this position|start application|i m interested|im interested)\b/.test(text));
    }

    function isRejectedNavigationText(text: string, allowApplyStart: boolean) {
      return /\b(cancel|back|previous|close|add)\b/.test(text)
        || /\b(submit|send application|complete application)\b/.test(text)
        || (!allowApplyStart && /\bapply now|apply for this job|apply for this position|start application\b/.test(text));
    }
  ...
  }, { allowApplyStart: options.allowApplyStart, adapterId }).catch(() => [] as Array<{ attributeValue: string; score: number }>);
}

async function clickFallbackNavigationControl(context: BrowserContext, page: Page, attributeValue: string) {
  const selector = `[data-gradlaunch-fallback-nav-id="${attributeValue}"]`;
  ...
  for (const frame of page.frames()) {
    ...
    const clicked = await frame.evaluate((value) => {
      ...
      const control = getSearchRoots()
        .map((root) => root.querySelector(`[data-gradlaunch-fallback-nav-id="${CSS.escape(value)}"]`))
...
async function clickObservedControl(context: BrowserContext, page: Page, controlId: string) {
  const selector = `[data-gradlaunch-control-id="${controlId}"]`;
  ...
  for (const frame of page.frames()) {
    ...
    const clicked = await frame.evaluate((targetId) => {
      ...
      const control = getSearchRoots()
        .map((root) => root.querySelector(`[data-gradlaunch-control-id="${CSS.escape(targetId)}"]`))
        .find((candidate): candidate is HTMLElement => candidate instanceof HTMLElement);
...
function didStageSignatureChange(
  before: Awaited<ReturnType<typeof getStageSignature>>,
  after: Awaited<ReturnType<typeof getStageSignature>>
) {
  return before.fingerprint !== after.fingerprint
    || before.progressText !== after.progressText
    || before.url !== after.url
    || before.title !== after.title
    || normalizeKey(before.visibleFieldLabels.join(" | ")) !== normalizeKey(after.visibleFieldLabels.join(" | "))
    || normalizeKey(before.requiredFieldLabels.join(" | ")) !== normalizeKey(after.requiredFieldLabels.join(" | "))
    || normalizeKey(before.controlLabels.slice(0, 12).join(" | ")) !== normalizeKey(after.controlLabels.slice(0, 12).join(" | "));
}
...
function isApplyStartText(value: string) {
  return /\b(apply now|apply for this job|apply for this position|start application|continue application|begin application|i'?m interested|im interested|apply)\b/i.test(value);
}
