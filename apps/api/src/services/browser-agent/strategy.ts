...
export function classifyPage(observation: BrowserAgentObservation): PageClassification {
  ...
  const hasApplyStartControl = observation.controls.some((control) => isApplyStartControlText(`${control.text} ${control.label}`));
  const hasProgressControl = observation.controls.some((control) => isApplicationProgressControlText(`${control.text} ${control.label}`));
  const hasHighSignalFormFields = observation.visibleFields.some((field) => isHighSignalApplicationField(`${field.label} ${field.context}`));
  const hasRequiredFormFields = observation.visibleFields.some((field) => field.required);
  const navigationFirstStage = hasProgressControl && !hasRequiredFormFields && !hasHighSignalFormFields;
  ...
  if (
    observation.pageState === "start"
    || (hasApplyStartControl && !hasRequiredFormFields && !hasHighSignalFormFields)
    || (!visibleFieldCount && hasProgressControl)
  ) {
    states.push({
      state: "start",
      confidence: !visibleFieldCount || hasApplyStartControl ? 0.9 : 0.82,
      blocking: false,
      reasons: [hasApplyStartControl
        ? "A clear Apply/Start Application control is visible before the real form fields."
        : "A forward application control is visible and the actual form fields are not exposed yet."]
    });
  }
  ...
  if (visibleFieldCount > 0 && !["login", "captcha"].includes(observation.pageState)) {
    ...
    states.push({
      state,
      confidence: hasRequiredFormFields ? 0.91 : navigationFirstStage ? 0.61 : 0.84,
      blocking: false,
...
export function rankActions(input: RankActionsInput): ActionScore[] {
  ...
  const hasProgressControl = observation.controls.some((control) => isApplicationProgressControlText(`${control.text} ${control.label}`));
  const hasHighSignalFormFields = observation.visibleFields.some((field) => isHighSignalApplicationField(`${field.label} ${field.context}`));
  const navigationFirstStage = hasProgressControl && !hasRequiredFields && !hasHighSignalFormFields;
  ...
  if (hasFields && !classification.blocking && classification.state !== "loading" && !navigationFirstStage) {
    scores.push({
      action: "fill",
  ...
  if (classification.state === "start") {
    scores.push({
      action: "click_next",
      score: hasApplyStartControl ? 92 : hasProgressControl ? 86 : 80,
      reasons: [hasApplyStartControl
        ? "A clear Apply/Start Application control is visible, so the agent should advance before trying to fill."
        : "This looks like a pre-form application stage, so advancing is safer than probing."]
    });
  }

  if (!hasFields && hasProgressControl && classification.state !== "resume_upload" && classification.state !== "submit" && classification.state !== "review") {
    scores.push({
      action: "click_next",
      score: 84,
      reasons: ["A forward application control is visible and no fillable form fields are exposed yet."]
    });
  }

  if (navigationFirstStage && classification.state !== "resume_upload") {
    scores.push({
      action: "click_next",
      score: hasApplyStartControl ? 88 : 80,
      reasons: ["Only low-signal or non-required fields are visible, while the main application path still points forward."]
    });
  }
...
function isApplyStartControlText(value: string) {
  ...
  if (/\b(sign in|signin|log in|login|google|email|password|forgot|create account|register|sign up)\b/.test(text)) {
    return false;
  }

  return /\b(apply now|apply for this job|apply for this position|start application|continue application|begin application|i m interested|im interested|submit your application|apply)\b/.test(text);
}

function isApplicationProgressControlText(value: string) {
  const text = normalizeKey(value);
  ...
  if (/\b(cancel|back|previous|close|dismiss|withdraw)\b/.test(text)) {
    return false;
  }

  return /\b(apply|start application|continue application|continue|next|proceed|review application|save and continue)\b/.test(text);
}

function isHighSignalApplicationField(value: string) {
  const text = normalizeKey(value);

  return /\b(first name|last name|full name|email|confirm your email|phone|mobile|city|location|address|linkedin|portfolio|website|resume|cv|cover letter|school|university|college|degree|experience|work authorization|sponsorship|salary|notice period)\b/.test(text);
}
...
async function collectFallbackObservation(page: Page) {
  const visibleFields: BrowserAgentObservation["visibleFields"] = [];
  const observation = await page.evaluate(() => ({
    url: window.location.href,
    title: document.title,
    pageText: (document.body?.innerText ?? "").replace(/\s+/g, " ").slice(0, 3500)
  })).catch(() => ({ url: page.url(), title: "", pageText: "" }));
